...
export function RestoreDialog({ conn, onClose }: Props) {
  ...
  async function handleStartRestore() {
    ...
    try {
      ...
      await startRestore(buildConfig(selectedConn, password, sshPassword), selectedDb, inputPath, Array.from(activeFlags), id);
    } catch (e) {
      setError(String(e));
      setJobStatus("idle");
    }
    ...
    <div className="fixed inset-0 z-50 flex flex-col bg-background">
      ...
      <div className="flex items-center gap-3 px-6 py-2 border-b shrink-0">
        ...
        <span className="text-xs font-mono flex-1 truncate">
          {inputPath ? inputPath.split("/").pop() : "No file selected"}
        </span>
        <button onClick={pickFile} className="px-3 py-1 text-xs border rounded hover:bg-muted transition-colors">
          Browse...
        </button>
      </div>
      ...
      <div className="flex items-center justify-end gap-3 px-6 py-3 border-t shrink-0">
        ...
        {jobStatus === "running" && (
          <span className="text-xs text-muted-foreground animate-pulse">Restoring...</span>
        )}
